...
var summerHtmlImageMapCreator = function (imageSrc, preExistingMaps, saveCb, cancelCb){
    'use strict';
    /* Utilities */
    var utils = {
        /**
    ...
    var app = (function () {
        var domElements = {
                wrapper: utils.id('mapWrapper'),
                svg: utils.id('svg'),
                img: utils.id('mapImg'),
                container: utils.id('mapImage'),
                map: null
            },
            state = {
                offset: {
                ...
                appMode: 'editing', // drawing || editing || preview
                currentType: null,
                editType: null,
                newArea: null,
                selectedArea: null,
        ...
        var instructions_info = (function () {
            var instructions = utils.id('instructions');
        ...
        $(domElements.container).mousemove(function (e){
            var offset = $(this).offset();
            var x = Math.round(e.pageX - offset.left);
            var y = Math.round(e.pageY - offset.top);
            cursor_position_info.set({x: x, y: y});
        });

        $(domElements.container).mouseleave(function (e){
            cursor_position_info.empty();
        });

        $(domElements.container).mousedown(function (e){
            var offset = $(this).offset();
        ...
        $(domElements.container).click(function (e){
            // if (state.currentType === "polygon") {
                var offset = $(this).offset();
        ...
        $(domElements.container).dblclick(function (e){
            var offset = $(this).offset();
            if (state.appMode === 'editing') {
                if (e.target.tagName === 'rect' || e.target.tagName === 'circle' || e.target.tagName === 'polygon') {
                    state.selectedArea = e.target.parentNode.obj;
                    info.load(state.selectedArea, e.pageX - offset.left, e.pageY - offset.top);
                }
            }
        });
        ...
        return {
            ...
            setIsDraw: function (arg) {
                state.isDraw = arg;
                arg ? instructions_info.set("'ESC' to cancel") : instructions_info.empty()
                return this;
            },
    ...
    Area.ATTRIBUTES_NAMES = ['NAME']; //'HREF', 'ALT', 'TITLE', 
    ...
    Area.createAreasFromHTMLOfMap = function (htmlStr) {
        ...
        while (true) {
            ...
            coords = coords.map(function (item) {
                return Math.round(Number(item)); //fixes any floating point numbers that may have been saved
            });
    ...
    Rectangle.prototype.onProcessDrawing = function (e) {
        var coords = utils.getRightCoords(e);
    ...
    Rectangle.prototype.onStopDrawing = function (e) {
        ...
        this.setCoords(this.dynamicDraw(coords.x, coords.y, e.shiftKey)).select();

        info.load(this, coords.x, coords.y - 20);
    ...
    Rectangle.prototype.toHTMLMapElementString = function () {
        ...
        return '<area class="area" shape="rect" coords="' // TODO: use template engine
            +
            this._coords.x + ', ' +
            this._coords.y + ', ' +
            x2 + ', ' +
            y2 +
            '"' + ' href="#"'
            /*(this._attributes.href ? ' href="' + this._attributes.href + '"' : 'href="#"')*/ +
            (this._attributes.alt ? ' alt="' + this._attributes.alt + '"' : '') +
            (this._attributes.title ? ' title="' + this._attributes.title + '"' : '') +
            (this._attributes.name ? ' data-name="' + this._attributes.name + '"' : '') +
            ' />';
    };
    ...
    Circle.prototype.onStopDrawing = function (e) {
        ...
        this.setCoords(this.dynamicDraw(coords.x, coords.y)).select();
    ...
    Circle.prototype.toHTMLMapElementString = function () {
        return '<area class="area" shape="circle" coords="' +
            this._coords.cx + ', ' +
            this._coords.cy + ', ' +
            this._coords.radius +
            '"' +
    ...
    Polygon.prototype.onAddPointDrawing = function (e) {
        var newPointCoords = utils.getRightCoords(e);
    ...
    Polygon.prototype.toHTMLMapElementString = function () {
        ...
        return '<area class="area" shape="poly" coords="' +
            str +
            '"' +
            // (this._attributes.href ? ' href="' + this._attributes.href + '"' : '') +
            // (this._attributes.alt ? ' alt="' + this._attributes.alt + '"' : '') +
    ...
    Polygon.getCoordsFromHTMLArray = function (htmlCoordsArray) {
        if (!Polygon.testHTMLCoords(htmlCoordsArray)) {
            throw new Error('These html-coordinates are not valid for a polygon');
        }
    ...
    var info = (function () {
        var form = utils.id('edit_details'),
            ...
            underlay = utils.id('form-underlay'),
            error = utils.id('naming_error'),
            // href_attr = utils.id('href_attr'),
            // alt_attr = utils.id('alt_attr'),
            // title_attr = utils.id('title_attr'),
            name_attr = utils.id('name_attr'),
            save_button = utils.id('save_details'),
            delete_button = utils.id('delete_shape'),
        ...
        function save(e) {
            e.preventDefault();
            if (!(/\S/.test(name_attr.value))) {
                error.innerHTML = "The name field is required.";
                error.style.display = 'block';
                console.log("no sir this is an error");
            }
            else {
        ...
        function setCoords(x, y) {
            form.style.left = (x + 25) + 'px';
            form.style.top = (y + 85) + 'px';
        }
        ...
        delete_button.addEventListener('click', delete_shape, false);
        ...
        //     e.stopPropagation();
        // }, false);
        name_attr.addEventListener('keydown', function (e) {
            e.stopPropagation();
        }, false);
        ...
        return {
            load: function (object, new_x, new_y) {
                ...
                name_attr.value = object._attributes.name ? object._attributes.name : '';
                utils.show(form);
                utils.show(underlay);
                if (new_x && new_y) {
                    x = new_x;
    ...
    if (preExistingMaps) {
        Area.createAreasFromHTMLOfMap(preExistingMaps);
    }

    /* Buttons and actions */
    var buttons = (function () {

        var all = utils.id('editor-nav').getElementsByTagName('button'),
            save = utils.id('save'),
        ...
        function selectEdit() {
            edit.classList.add(Area.CLASS_NAMES.SELECTED);
        }
        ...
        function onSaveButtonClick(e) {
            var data = app.getHTMLCode(true);
            saveCb(data);
            // if (data !== '0 objects') {
            //     localStorage.setItem('imageMapData', data);
            //     console.log('saved');
            // }
            e.preventDefault();
        }
        ...
        $(document).ready(function () {
            ...
            exit.addEventListener('click', onExitButtonClick, false);
